...
export const AdminPurchaseOrders = () => {
  ...
  const columns = [
    ...
    { header: 'Awarded Supplier', accessor: 'supplier_name', render: row => (
      <div>
    ...
    { header: 'Total PO Amount', accessor: 'dec_Final_Amount', render: row => (
  ...
  return (
    <div>
      ...
      <Table columns={columns} data={purchases} searchPlaceholder="Search POs by PO number, store, supplier..." />
      ...
      {selectedPO && (
        <Modal
          ...
          title={`Purchase Order Document: ${selectedPO.po_number}`}
          maxWidth="700px"
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '16px', background: 'var(--color-surface-hover)', borderRadius: '8px', border: '1px solid var(--color-border)' }}>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>College / Hostel Store</div>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>Issued To Supplier</div>
                <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--color-purple-text)' }}>{selectedPO.supplier_name}</div>
                <div style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: '2px' }}>PO Date: {selectedPO.dte_Purchase_Date}</div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
              ...
              <div className="card" style={{ padding: '12px 16px' }}>
                <span style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>Freight Charge</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: 'var(--color-warning-bg)', borderRadius: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <div style={{ fontWeight: 600, fontSize: '0.85rem', color: 'var(--color-warning-text)' }}>Fulfillment Status</div>
